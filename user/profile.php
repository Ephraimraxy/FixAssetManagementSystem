...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_profile'])) {
    ...
    if (!empty($phone) && !preg_match('/^[0-9\+\-\s()]*$/', $phone)) {
        $error_message = 'Invalid phone number format. Please use numbers and valid characters only.';
    }
            ...
            if ($stmt->rowCount() > 0) {
                ...
                if (!empty($current_password) && !empty($new_password)) {
                    ...
                    if (!password_verify($current_password, $user['password'])) {
                        ...
                        $stmt = $pdo->prepare('
                            ...
                            SET username = ?, email = ?, full_name = ?, phone = ?, 
                                ...
                                theme_preference = ?, language_preference = ?, status = ?,
                                updated_at = NOW() 
                            WHERE user_id = ?
                        ');
                        if ($stmt->execute([$username, $email, $full_name, $phone, $department, $position, $hashed_password, $_POST['theme_preference'], $_POST['language_preference'], $_POST['status'], $user_id])) {
                    ...
                    $stmt = $pdo->prepare('
                        ...
                        WHERE user_id = ?
                    ');
                    if ($stmt->execute([$username, $email, $full_name, $phone, $department, $position, $theme_preference, $language_preference, $status, $user_id])) {
    ...
    if (isset($_FILES['profile_picture']) && $_FILES['profile_picture']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($file_type, $allowed_types)) {
            ...
            if (!file_exists($upload_dir)) {
                mkdir($upload_dir, 0777, true) or die('Failed to create upload directory');
            }
            ...
            // Generate unique filename and ensure it's safe
            $file_name = $user_id . '_' . time() . '_' . pathinfo($_FILES['profile_picture']['name'], PATHINFO_EXTENSION);
            ...
            <div class="card shadow-sm">
                <div class="card-header bg-white">
                    <h5 class="card-title mb-0">
                        <i class="fas fa-edit me-2 text-primary"></i> Account Settings
                    </h5>
                </div>
                <div class="card-body">
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="theme_preference" name="theme_preference">
                                    <option value="system" <?php echo $user['theme_preference'] === 'system' ? 'selected' : ''; ?>>System Default</option>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="status" name="status" <?php echo $user['role'] === 'admin' ? '' : 'disabled'; ?>>
                                    <option value="active" <?php echo $user['status'] === 'active' ? 'selected' : ''; ?>>Active</option>
                                    <option value="inactive" <?php echo $user['status'] === 'inactive' ? 'selected' : ''; ?>>Inactive</option>
                                </select>
                                <div class="form-text">Only administrators can change account status</div>
                            </div>
                        </div>
