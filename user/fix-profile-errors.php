<?php
// This script fixes all errors in the profile.php file
...
copy('profile.php', 'profile.php.bak-' . date('Y-m-d-H-i-s'));

// Get the current profile.php content
$profile_content = file_get_contents('profile.php');

// Fix 1: Fix the TypeError on line 213 - Cannot access offset of type string on string
// Add isset() checks for all array accesses
$profile_content = str_replace(
    "echo htmlspecialchars(\$user['full_name']);",
    "echo isset(\$user['full_name']) ? htmlspecialchars(\$user['full_name']) : 'User';",
    $profile_content
);

$profile_content = str_replace(
    "\$role_text = (\$user['role'] === 'admin') ? 'Administrator' : 'User';",
    "\$role_text = (isset(\$user['role']) && \$user['role'] === 'admin') ? 'Administrator' : 'User';",
    $profile_content
);

// Fix 2: Fix any other potential array access issues
...
// Fix 3: Fix the syntax error with unexpected token "else"
// This usually happens when there's a missing closing brace or mismatched braces
// Create a clean version of the file with proper structure

// Save the fixed content back to profile.php
file_put_contents('profile.php', $profile_content);

// Create a completely new profile.php file with clean structure
$new_profile = '<?php
require_once \'../includes/config.php\';
        ...
        <div class="col-md-4 mb-4">
            <div class="card shadow-sm">
                <div class="card-body text-center">
                    ...
                    <h5 class="mb-1"><?php echo isset($user[\'full_name\']) ? htmlspecialchars($user[\'full_name\']) : \'User\'; ?></h5>
...
// Check if we should completely replace the file
if (strpos($profile_content, 'unexpected token "else"') !== false || strpos($profile_content, 'Cannot access offset of type string on string') !== false) {
    file_put_contents('profile.php', $new_profile);
    ...
    echo '<h2 style="text-align: center;">Profile.php Completely Fixed!</h2>';
    echo '<p>Your profile.php file had multiple serious errors that required a complete rewrite. The file has been completely replaced with a clean, error-free version.</p>';
    echo '<p>The following issues have been fixed:</p>';
    echo '<ul>';
    echo '<li><strong>TypeError:</strong> Fixed "Cannot access offset of type string on string" error by adding proper checks for array access</li>';
    echo '<li><strong>Syntax Error:</strong> Fixed "unexpected token else" error by restructuring the code</li>';
    echo '<li><strong>Array Access:</strong> Added isset() checks for all array accesses to prevent future errors</li>';
    echo '<li><strong>Backup:</strong> Created a backup of your original file (profile.php.bak-[timestamp])</li>';
    echo '</ul>';
} else {
    echo '<div style="background-color: #d4edda; color: #155724; padding: 20px; margin: 20px; border-radius: 5px; font-family: Arial, sans-serif;">';
    echo '<h2 style="text-align: center;">Profile.php Fixed!</h2>';
    echo '<p>The TypeError in your profile.php file has been fixed. The following changes were made:</p>';
    echo '<ul>';
    echo '<li>Added isset() checks for all array accesses to prevent "Cannot access offset of type string on string" errors</li>';
    echo '<li>Created a backup of your original file (profile.php.bak-[timestamp])</li>';
    echo '</ul>';
}
